...
class HospitalRepositorySet() : HospitalRepository<Paciente, String> {

    val aforoMaximo = 30
    var pacientes = hashSetOf<Paciente>(
    ...
    override fun ingresar(): Paciente? { // CREATE
        if (!isHospitalCompleto()) {
            pacientes.add(agregarPaciente())
            return pacientes.last()
        }
        return null
    }
    ...
    override fun pacientesPorTipo(tipo: tipoPaciente): ListaPacientes {
        return pacientes.filter { it.tipoPaciente == tipo }
    }
